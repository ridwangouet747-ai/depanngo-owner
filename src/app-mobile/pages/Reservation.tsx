...
import { ArrowLeft, Wrench, MapPin, User, Clock, Info, CalendarDays } from "lucide-react";
...
const TIME_SLOTS = [
  { id: "08:00", label: "08h00 – 10h00", period: "Matin" },
  { id: "10:00", label: "10h00 – 12h00", period: "Matin" },
  { id: "12:00", label: "12h00 – 14h00", period: "Midi" },
  { id: "14:00", label: "14h00 – 16h00", period: "Après-midi" },
  { id: "16:00", label: "16h00 – 18h00", period: "Après-midi" },
];

const WAVE_LOGO = (
  <svg viewBox="0 0 48 48" fill="none" className="w-full h-full">
    <rect width="48" height="48" rx="12" fill="#1877F2" />
    <text x="24" y="32" fontSize="20" fontWeight="900" fill="white" textAnchor="middle" fontFamily="Arial">W</text>
    <path d="M10 30 Q18 22 24 30 Q30 38 38 30" stroke="white" strokeWidth="3" fill="none" strokeLinecap="round" />
  </svg>
);
...
const OM_LOGO = (
  <svg viewBox="0 0 48 48" fill="none" className="w-full h-full">
    ...
    <text x="24" y="29" fontSize="14" fontWeight="900" fill="white" textAnchor="middle" fontFamily="Arial">OM</text>
  </svg>
);
...
const MTN_LOGO = (
  <svg viewBox="0 0 48 48" fill="none" className="w-full h-full">
    ...
    <text x="24" y="21" fontSize="11" fontWeight="900" fill="#1A1A2E" textAnchor="middle" fontFamily="Arial">MTN</text>
    <text x="24" y="34" fontSize="10" fontWeight="700" fill="#1A1A2E" textAnchor="middle" fontFamily="Arial">MoMo</text>
  </svg>
);
...
export default function Reservation() {
  ...
  const [selectedPayment, setSelectedPayment] = useState<PayMethod | null>(null);
  ...
  async function handlePay() {
    if (!selectedPayment || !selectedDate || !selectedSlot) return;
    setLoading(true);
    try {
      // Create a transaction first
      const { data: tx, error: txError } = await supabaseExt
        .from("transactions")
        .insert({
          client_id: (await supabaseExt.auth.getUser()).data.user?.id,
      ...
      const result = await initiatePayment({
        transactionId: tx.id,
        provider: selectedPayment === "wave" ? "wave" : selectedPayment === "orange" ? "orange_money" : "mtn_momo",
        amountFcfa: deposit,
        type: "deposit",
        phoneNumber: "", // Will be collected in a real flow
      });

      if (result.success) {
        toast.success("Réservation confirmée !", {
          description: `${fmt(deposit)} payé — ${formatDateFR(selectedDate)} à ${TIME_SLOTS.find((s) => s.id === selectedSlot)?.label}`,
        });
        navigate("/app/missions");
      } else {
        toast.error("Paiement échoué", { description: result.error });
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F5F5F5] flex flex-col max-w-[430px] mx-auto pb-40">
      ...
      <header className="flex items-center px-6 pt-12 pb-4">
        <button
          onClick={() => (step === "payment" ? setStep("schedule") : navigate(-1))}
        ...
        <h1 className="flex-1 text-center font-black text-lg text-gray-900 mr-10">
          {step === "schedule" ? "Choisir date et heure" : "Confirmer la réservation"}
        </h1>
      </header>
      ...
      <div className="px-6 space-y-4">
        ...
        <div className="flex gap-2 mb-2">
          <div className={`h-1.5 flex-1 rounded-full ${step === "schedule" ? "bg-orange-500" : "bg-orange-200"}`} />
          <div className={`h-1.5 flex-1 rounded-full ${step === "payment" ? "bg-orange-500" : "bg-gray-200"}`} />
        </div>
        ...
        {step === "schedule" ? (
          <>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-sm text-gray-900">
                    Créneaux pour le {formatDateFR(selectedDate)}
                  </span>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <Clock size={16} className={active ? "text-orange-500" : "text-gray-400"} />
                          <span className="font-bold text-sm text-gray-900">{slot.label}</span>
                        </div>
                        <span className="text-[10px] font-bold text-gray-400 uppercase">{slot.period}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            <button
              ...
              className="w-full h-14 bg-orange-500 text-white font-black rounded-[14px] flex items-center justify-center active:scale-95 transition-transform disabled:opacity-40 mt-4"
              style={{ boxShadow: selectedDate && selectedSlot ? "0 4px 20px rgba(232,89,12,0.3)" : "none" }}
            >
              Continuer
            </button>
          </>
                ...
                {selectedDate && selectedSlot && (
                  <div className="flex items-center gap-3 text-orange-500">
                    ...
                    <span className="font-semibold text-sm">
                      {formatDateFR(selectedDate)} — {TIME_SLOTS.find((s) => s.id === selectedSlot)?.label}
                    </span>
                  </div>
                )}
              </div>
      ...
      {/* Bouton fixe en bas (étape paiement uniquement) */}
      {step === "payment" && (
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white/80 backdrop-blur-md border-t border-gray-200 px-6 pt-4 pb-8 z-50">
          <button
            ...
            style={{ boxShadow: selectedPayment ? "0 4px 20px rgba(232,89,12,0.3)" : "none" }}
          >
            {loading ? "Paiement en cours..." : `Payer l'acompte — ${fmt(deposit)}`}
          </button>
